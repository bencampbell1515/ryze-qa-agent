...
- **Persona context overflow from accumulated screenshot base64 (FIXED)** — brand-purist and forensic-technician hit `400: prompt is too long: >200k tokens`. Root cause: each `screenshot` tool result embeds full base64 (~6k tokens) in the messages array and it persists for every subsequent API call in the session. A session with ~15 screenshots = 90k+ stale image tokens. Fix: `pruneOldScreenshotImages()` in `agent-loop.ts` strips base64 from all but the 2 most recent screenshot results before each API call. Also added `MAX_SUMMARY_CHARS = 4000` cap on `buildFindingsSummary()` in `persona-runner.ts` as defence-in-depth for cross-session summary growth.
