...
| runner-daemon.ts | **Long-lived background process**, started by launchd (`com.ryzewith.qaagent.plist`). Listens to `runs` + `diffRequests` collections in Firestore. For each requested run: writes scan config to `data/.ryze-scan-config.json`, spawns `npm run full-audit` (detached process group), streams stdout/stderr + progress + URL counts to Firestore, uploads HTML/PDF/scored-bugs.json/suppressed.html to Storage on completion. **Worktree N1 (2026-05-29):** also uploads `findings.jsonl`, `uncertain-findings.jsonl`, `suppressed-findings.jsonl`, `hygiene.jsonl`, and `output/crops/{runId}/*.png` to `reports/{runId}/` + stamps 10 new path/count fields on the run doc (`findingsJsonPath`, `cropsPrefix`, `findingsCount`, etc.). All file uploads guarded by `existsSync`; counts default to 0. For each diff request: downloads both bug JSONs, computes exact-match diff, then sends unmatched piles to Haiku for semantic matching. Cancellation: process-group SIGINT → SIGTERM (8s) → SIGKILL (15s) escalation. Orphan recovery on startup marks any `running` / `cancel-requested` docs as failed. **See root CLAUDE.md "Web UI + Runner daemon" for the full picture.** |
| upload-directory.ts | **Worktree N1 (2026-05-29).** `uploadDirectoryRecursive(bucket, localDir, remotePrefix, opts)`. Walks `localDir` recursively, uploads files in parallel (default concurrency 8) preserving relative structure under `remotePrefix`, returns `{ uploaded, skipped }`. Missing dir → empty result, no throw. Per-file upload failure lands in `skipped` (logged warn), batch continues. Bucket is injected → unit-testable against a mock. Used by `runner-daemon.ts` to upload the `output/crops/{runId}/` tree. |
