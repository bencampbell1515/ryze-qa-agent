...
## Shipped (2026-05-29, session) — Full QA-agent rebuild executed across 12 worktrees + Phase 2 dashboard integration

Executed the full parallel-worktree rebuild from preflight bundle to deployed production. Started with a fresh tarball at `~/Downloads/qa-rebuild-preflight.tar.gz`, ended with `live-qa-agent.web.app` rendering the new tabbed Findings section against real Firebase data from a daemon running with rebuild features on by default. Sequence:

**Preflight + Phase 0:** extracted bundle, restored case-conflicting `setup.md`, renamed new guide to `PREFLIGHT.md`, filled `config/canonical-record.json` with the real Austin HQ address, committed `dae8a41`. Added `data/tmp/` and `.lycheecache` to `.gitignore`. Widened `test:unit` glob to cover `tests/cross-page/ tests/discovery/ tests/visual-regression/ tests/journeys/`. Installed lychee 0.24.2 from the GitHub release tarball (aarch64-apple-darwin, SHA256 verified) into `~/.local/bin` — no Homebrew, no Rust toolchain, no sudo. Symlinked VS Code's bundled `code` CLI into `~/.local/bin` so subsequent `code <path>` worked without the Command Palette dance. Wrote `docs/operational-constraints.md` capturing the Cloudflare O2O TLS-fingerprint constraint surfaced by worktree B's lychee dry-run.

**Phase 1 — parallel worktree batch (A → L):** 7 initial worktrees (A, B, C, D, E, F, G), then H, M (split into M1/M2), I, J, K, L. Each followed the same recipe — verify (test:unit count + scope + boundary grep + targeted file inspections per brief), squash-merge via `gh pr merge N --squash`, pull main, re-test, clean up worktree + branch (local + origin). Test count progression on main: **476 (baseline) → 728 → 764 (H) → 852 (A) → 888 (C) → 956 (D) → 1112 (M1) → 1156 (M2) → 1272 (I) → 1380 (J) → 1476 (K) → 1556 (L)**.

PRs #2 (C) and #3 (D) needed rebases — same pattern: `package.json` dependencies block collision with F's earlier `pixelmatch`/`pngjs` additions. Pure union-merge resolution (`eld` + `fastest-levenshtein` + `pixelmatch` + `pngjs` + originals); `test:unit` glob conflict (C only) resolved to main's wider version. After rebase + `npm install` regenerated the lockfile, both merged cleanly.

**Phase 2 — daemon + dashboard integration (N1, N2):** N1 wired the daemon to upload `findings.jsonl` / `uncertain-findings.jsonl` / `suppressed-findings.jsonl` / `hygiene.jsonl` + `output/crops/{runId}/*.png` to Firebase Storage + stamp 10 new fields on the `runs/{id}` Firestore doc — purely additive, existing 5 uploads unchanged (verified: `48 insertions, 0 deletions`). N2 added `FindingsSection` + `FindingCard` + `FindingsList` + `HygieneEntry` components to the dashboard, plus `web/lib/findings-parse.ts` + vitest (web had no test runner before). Final root test count: **1592 passing**, + 19 vitest tests in `web/`.

**Production turn-on:** `com.ryzewith.qaagent.plist` updated with `RYZE_ENABLE_RUBRICS=1`, `RYZE_ENABLE_GATE=1`, `RYZE_ENABLE_TWO_JUDGE=1` in `EnvironmentVariables` — committed as `5d560ed`, copied to `~/Library/LaunchAgents/`, daemon bootout+bootstrap'd (first bootstrap collided with bootout cleanup → `Input/output error` → retry succeeded). Dashboard built (Next 16 static export, 51 files) and deployed to `live-qa-agent.web.app`.

**Key insights:**
- **`emitBug` dual-write is byte-identical by construction, not by translation.** The helper passes the `BugInstance` to `bugs.add()` **unconditionally and verbatim** before optionally building a Finding. Means the legacy `bugs.jsonl` stream is mathematically unchanged regardless of whether the v2 Finding context is threaded. This guarantee held across M1's 9 emit-site migration and M2's 27 — all 36 swaps preserved the BugInstance object literal exactly.
- **Forced tool-use beats text-parsing for structured LLM output.** Worktree I's rubric runner uses `tool_choice: { type: 'tool', name: 'submit_verdicts' }` and finds the `tool_use` block in the response — never JSON.parse/regex/text matching. K's two-judge layered on top of J's gate via `Promise.allSettled` with `uncertain` as a non-vote, so an erroring judge degrades to single-judge rather than crashing.
- **Mechanical migrations stay safe when the wrapper is small and the pattern is uniform.** M2 swapped `bugs.add({` → `emitBug(bugs, ctx, {` and `});` → `}, { title });` across 10 files / 27 sites. Per-file `-bugs.add` count = `+emitBug` count for every file; the only field-line changes inside object literals were the trailing closing-pattern shift. Same proof shape as M1 — easy to verify without running the audit.
- **macOS case-insensitive FS bit twice in one session.** First: tarball's `SETUP.md` silently overwrote existing `setup.md` (same inode on APFS). Recovery attempt then bit again: `git show HEAD:setup.md > setup.md` followed by `mv SETUP.md PREFLIGHT.md` moved the just-restored content. Fix was extract to `/tmp` and copy with explicit different name.
- **launchctl plist changes need bootout+bootstrap, not kickstart.** `kickstart -k` reloads the process but reads the cached env from when the service was first loaded. To pick up new `EnvironmentVariables`, `bootout` (unloads) then `bootstrap` (reloads). First bootstrap after bootout can fail with `Input/output error` (exit 5) due to a race with cleanup — retry once. Confirmed by checking `launchctl print | grep environment` after.
- **`gh pr view --json mergeable` returns `UNKNOWN` on first call**, lazily computes, ready on second. Forcepushes also leave a stale `CONFLICTING` for a few seconds. Always poll: `until result != "UNKNOWN" do sleep 2`.
- **Worktree created before brief commit ships** is a recurring footgun. The worktree branches off `main` at the moment of `git worktree add`; a subsequent commit to main (the brief) is invisible to the worktree until the session inside it `git pull`s. Three options: commit brief first then create worktree; document the `git pull` in the kickoff prompt; or detect at session start.
- **Cloudflare O2O is a TLS-layer gate, not a UA gate.** Worktree B's lychee dry-run produced 38/50 pages 403'd — lychee's reqwest fingerprint isn't trusted regardless of UA spoofing. Same paths return 200 via system Chrome under O2O. Documented in `docs/operational-constraints.md` as authoritative guidance for future worktrees: any HTTP-touching check must flow through Playwright's fetch path (`page.goto`, `page.request.fetch`); Node `fetch`/`axios`/Rust/Go binaries will 403. Cloud Run migration must either run a real Chrome in-container or get the egress IP on Cloudflare's trust list.

## Shipped (2026-05-27, session) — Live dashboard telemetry + denoise + dr-marcus-chen as meta-analysis

Live audit produced a UI showing "0 bugs found" and "82% complete" with substantial work still remaining. Investigation found four orthogonal display bugs plus one architectural misconfig (dr-marcus-chen running as a page-level persona despite his prompt explicitly saying not to submit page-level findings). Worktree-isolated all fixes from the running audit, committed after the audit finished, then merged + pushed + deployed.

**Findings + fixes:**
- ~~`bugCount` stayed `—` for hours during a run~~ — daemon only wrote it at completion from `scored-bugs.json`. Now `readLiveBugCount()` sums `bugs.jsonl` + `discoveries.jsonl` line counts every flush tick, falls through to scored count at the end.
- ~~Progress percent stalled at 82% with much work remaining~~ — orchestrate was compressed into 85→100% of the bar even though it realistically takes 10–60 min. Rebanded: queued=3, crawl=8, audit=20 (computeAuditPercent stretches 20→58), orchestrate=60 (occupies 60→100).
- ~~Persona never reached 100% in the daemon's percent calc~~ — `persona-runner.ts` `while` loop broke *before* logging when `unvisited.length === 0`, leaving `current` pinned at the last session's SESSION_BUDGET=7 leftover. Added an explicit `Session N+1: 0 URLs remaining` log line after the loop.
- ~~"254 URLs scanned vs 231 found" — counter exceeded the discovered total~~ — daemon summed `(initial - current)` across 5 personas, all walking the same URL list, so the theoretical max was 5 × urlCount. Now parses `[persona] visited <url>` log lines (emitted from persona-runner) and tracks a union Set. Fallback path also caps at `urlCount`.
- ~~Elapsed time was frozen between log flushes~~ — `duration()` is pure but never re-rendered. Added `useEffect(setInterval, 1000)` ticker in `RunDetail.tsx` while status is running/requested. Pinned at completion.
- ~~dr-marcus-chen ran as a page-level persona despite his "no page-level findings" mandate~~ — he was in `PERSONA_BATCHES` and got handed the full URL list. Burned ~$1–3 of Haiku tokens per run producing nothing useful, while his actual meta-analysis output was always missing. Removed from PERSONA_BATCHES. Added `runMetaAnalysis()` step in `orchestrate.ts` that reads `dismissed.jsonl` + `report-history.jsonl` + this run's fingerprints, sends one Haiku call with his persona file, writes `output/system-health-<date>.md`. Daemon uploads as `systemHealthPath` artifact.
- ~~`console:error` + `js:pageerror` accounted for ~83% of bug entries (~13k of 15k per run) and dominated `validate.ts` for 20–40 min~~ — these are always third-party noise in headless+blocked-GTM context (Popper.js, analytics, jQuery). Removed `attachConsoleListeners()` call from `tests/crawl.spec.ts`. Function preserved in `tests/checks/console.ts` for the day audits move to a trusted browser context.
- ~~`validate.ts` had no NOISE_RULE_IDS filter and sent every bug to Haiku at pLimit(20)~~ — defense-in-depth: added a `NOISE_RULE_IDS` set + pre-filter so a stale `bugs.jsonl` from before this fix won't re-flood Haiku. Same rule IDs now live in THREE files (report.ts, orchestrate.ts, validate.ts) — must stay aligned until a shared `src/noise-config.ts` exists.

**Process win — isolated worktree saved the audit:** Work happened in `.worktrees/dashboard-fixes` on a `dashboard-fixes` branch while the daemon kept running from main. Zero risk of disrupting the in-flight scan. After it completed cleanly (69 bugs, artifacts uploaded), merged dashboard-fixes into main, pushed, kickstarted the daemon, built+deployed the web UI. End-to-end verification: new daemon PID, fresh `Last-Modified` on hosting, CSP/HSTS headers intact.

**Key insights:**
- **Marcus was wired wrong from day one, not "still being tuned".** His persona file says explicitly *"You do NOT submit page-level bug findings. Your job is to evaluate the QA system itself each run."* — yet `PERSONA_BATCHES` handed him the URL list and `runPersona()` walked him through it. The fix isn't to coerce his prompt; it's to give him a different invocation surface. When a persona's stated mandate and the code's invocation pattern diverge, fix the invocation, not the prompt.
- **The dashboard's "live" counters were a polling/snapshot mismatch.** Daemon flushed every 750ms but only wrote `bugCount` once at the end; URL counter summed without union; elapsed string was pure-functional. Three subtly different bugs sharing one symptom ("static numbers"). Diagnosing each took reading both the writer (daemon) and the renderer (UI) — never just one side.
- **Killing noise at the collector source recovers 20–40× more wall-clock than filtering downstream.** Removing `attachConsoleListeners()` shrinks `bugs.jsonl` from ~15k to ~1.5k entries. The downstream NOISE_RULE_IDS filters in report.ts/orchestrate.ts/validate.ts are defense-in-depth, but the *real* cost is `validate.ts` calling Haiku once per raw bug at pLimit(20). Filter at the source whenever the noise is structurally identifiable; downstream filters are belt-and-suspenders only.
- **Progress bands need to reflect real wall-clock proportions.** A 5-phase pipeline shouldn't map (5/9/30/30/100) when phase 5 alone takes longer than phases 1–4 combined. If you're tempted to map orchestrate to "90 → 100", that's a sign the phase needs its own band.
- **Worktree + branch + delayed merge is the right pattern for editing code while it's running.** Both worktrees share `.git/` so commits are visible from either side, but working trees are independent. The running audit was completely insulated; main's pre-existing dirty state was unaffected until we explicitly committed it as a prep commit before merging. Zero risk of disruption — and the merge surfaces the conflict potential before pushing.
- **iPad Safari first-load is slow on Firebase static-export SPAs.** Reported as "stuck loading"; cleared itself on retry. Likely combination of ITP cookie partitioning slowing reCAPTCHA v3, plus Safari's slower JS parse on cold start. Not a bug; just slow. Subsequent loads are fast.
